...
import std.range : take, drop, iota;


int[] digits(string digitString) {
  if (digitString.any!(c => !c.isDigit)) {
    throw new Exception("Non-digit characters in input.");
  }
  return digitString.map!(c => c.to!string.to!int).array;
}


int[][] slice(string digitString, size_t span) {
  if (span > digitString.length) {
    throw new Exception("Span longer than provided string.");
  }
  int[][] result = [];
  foreach (offset; iota(0, digitString.length - span + 1)) {
    result ~= digitString.digits.drop(offset).take(span);
  }
  return result;
}
