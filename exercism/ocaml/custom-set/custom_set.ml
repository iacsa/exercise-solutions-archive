...
module Make(T: ITEM) = struct
  ...
  let of_list : T.t list -> t = List.fold ~init:empty ~f:add

  let to_list : t -> T.t list = Fn.id

  let union (set1: t) (set2: t) : t = List.fold ~init:set1 ~f:add set2
  ...
  let is_subset (set1: t) (set2: t) : bool =
    union set1 set2 = set2

  let is_disjoint (set1: t) (set2: t) : bool =
    intersect set1 set2 = []

  let is_member : t -> T.t -> bool = List.mem

  let is_empty (set: t) : bool = set = []
